...
def queueSubtask(commands, title, service="", limits=None, metadata=None, envkey=None):
    """
    ...
    for argv in commands:
        cmd_str = " ".join(argv)
        tclCmds.append(f"""RemoteCmd {{{cmd_str}}} {service_str} {tags_str} {envkey_str}""")
...
def queueChunkTask(node, taskCommand, config, tags=None, reqPackages=None, environment=None):
    ...
    for iteration in range(nbBlocks):
        ...
        taskArgs = taskInfo.get_kwargs()
        # limits, envkey
        taskArgs['limits'] = taskInfo.limits
        taskArgs['envkey'] = taskInfo.envkey
        # Commands
        commands = taskInfo.get_commands()
        queueSubtask(commands, **taskArgs)
